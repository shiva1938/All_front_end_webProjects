document.querySelector("#submit").addEventListener("click", () => {
    var hex_numbers = ["0", "1", "2", "3", "4", "5", "6", "a", "b", "c", "d", "e", "f"];
    var hexcode1 = "";
    var hexcode2 = "";

    for (let i = 0; i < 6; i++) {
        random_idx = Math.floor(Math.random() * hex_numbers.length);
        hexcode1 += hex_numbers[random_idx];
        random_idx = Math.floor(Math.random() * hex_numbers.length);
        hexcode2 += hex_numbers[random_idx];
    }
